...
if len(sys.argv) > 1:
  art = sys.argv[1]
  url = "http://www.goear.com/search/%s"%(art)
  try: html = str(urllib2.urlopen(url).read())
    ...
    number_tema=0
    while number_tema == 0:
      pag = int(raw_input('elija el numero de pagina q quiera: '));print "\n"
      # Estructura el listado de temas
      html = str(urllib2.urlopen( "%s/%d"%(url,pag)).read() )
    ...
    try:
      ...
      print "No se descargo el archivo / mala elección"
    else:
      print "\t\033[1;31mdescarga finalizada\033[1;00m"
else: print """
